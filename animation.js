const CSP = (rows, column, solution) => {
/**
  * Checking Constraint
  * @param {int} rows - CountRow
  * @param {int} column - CountColumn
  * @param {int} solution - prevSolution[SolutionIndex]
  * @returns {bool} True if in column j, no queen is placed from 0 to rows - 1
*/
 ...
 * */
const BoardGenerator = (n, totalSolutions, index) => {
    /**
   * Generating the Chess Board
   * @param {int} dimensions - For the Board Dimesions
   * @param {Object[[]]} totalSolutions - Two Dimesional Array
   * @param {int} index - Accessing the first dimension of totalsolutions   
   */
  const boxSize = 50,
    boardDimension = n,
...
const Check_Step = () => {
    ...
    if (solIndex === prevSolutions.length - 1) {
      ...
      if (countRow === n) { // finish checking the last row
        ...
        for (let i = 0; i < prevSolutions.length; i++) {
          BoardGenerator(n, prevSolutions, i);
        }
      };
  ...
  if (countRow > 0 && countRow < n && countCol <= n) {
    ...
    if (CSP(countRow, countCol, solution)) { //meet constraints -> place queens
  ...
  if (countRow === 0 && countCol < n) {
    ...
    if (CSP(countRow, countCol, solution)) {
